...
def make_square(im, fill_color=0):
    x, y = im.size
    size = max(x, y)
    new_im = Image.new('1', (size, size), fill_color)
...
def main():
    ...
    key = args.key
    message_image = make_square(message_image)
    message_image.show()
    ...
    secret_image = secret_image.rotate(r1)
    ciphered_image = ciphered_image.rotate(r2)
